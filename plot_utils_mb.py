...
def filter(exps_data, filters={}, remove=False):
    ...
    if filters:
        for i, exp in enumerate(exps_data):
            if not remove:
                ...
                keep_array.append(all([((filter_key not in exp['flat_params']) or (
                            (filter_key in exp['flat_params']) and (exp['flat_params'][filter_key] != filter_val)))
                                       for filter_key, filter_val in filters.items()]))
...
def group_by(exp_data, group_by_key=None):
    ...
    for exp in exp_data:
        if group_by_key == 'exp_name':
            ...
            if str(exp['flat_params'][group_by_key]).split('_')[0] == 'ppo':
                key_str = 'promp'
            elif str(exp['flat_params'][group_by_key]).split('_')[0] == 'trpo':
                key_str = 'maml'
            if key_str == 'def':
                key_str = str(exp['flat_params'][group_by_key]).split('_')[3]
