...
class Logger(object):
    ...
    def save_with_numpy(self, data_dict, d='np_data', path='arr', ext='.npz'):
        ''' save a dict of numpy arrays as npz '''
        os.makedirs(osp.join(self._snapshot_dir, d), exist_ok=True)
        filename = osp.join(self._snapshot_dir, d, path + ext)
        np.savez_compressed(filename, **data_dict)
    ...
    def dump_tabular(self, *args, **kwargs):
        ...
        if len(self._tabular) > 0:
            if self._log_tabular_only:
                table_printer.print_tabular(self._tabular)
            else:
