    ...
    def __init__(self, task_list):
        ...
        for i, task in enumerate(task_list):
            ...
            # TODO: this could cause flaws in task_idx if SawyerReachPushPickPlace6DOFEnv/SawyerReachPushPickPlaceWall6DOFEnv is not the first environment
                self._task_envs.append(task(multitask=False, obs_type='with_goal', random_init=False, fix_task=True, task_idx=i%3))
            else:
                self._task_envs.append(task(multitask=False, obs_type='with_goal', random_init=False))
