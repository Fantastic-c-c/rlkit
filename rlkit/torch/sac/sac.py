...
class ProtoSoftActorCritic(MetaTorchRLAlgorithm):
    ...
    def prepare_encoder_data(self, obs, act, rewards):
        ...
        if self.sparse_rewards:
            rewards = self.env.sparsify_rewards(rewards)
    ...
    def _take_step(self, indices, obs_enc, act_enc, rewards_enc):
        ...
        rewards_flat = rewards.contiguous().view(self.batch_size * num_tasks, -1)
        # scale rewards for Bellman update
        rewards_flat = rewards_flat * self.reward_scale
        terms_flat = terms.contiguous().view(self.batch_size * num_tasks, -1)
