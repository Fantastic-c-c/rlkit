...
class PEARLSoftActorCritic(MetaRLAlgorithm):
    ...
    def _take_step(self, indices, context):
        ...
        # save some statistics from training
        if self.train_statistics is None:
            # train loop should set this to None.
            # this way, these statistics are only computed for one batch.
            self.train_statistics = OrderedDict()
            if self.use_information_bottleneck:
                z_mean = np.mean(np.abs(ptu.get_numpy(self.agent.z_means[0])))
                z_sig = np.mean(ptu.get_numpy(self.agent.z_vars[0]))
                self.train_statistics['Z mean train'] = z_mean
                self.train_statistics['Z variance train'] = z_sig
                self.train_statistics['KL Divergence'] = ptu.get_numpy(kl_div)
                self.train_statistics['KL Loss'] = ptu.get_numpy(kl_loss)

            self.train_statistics['QF Loss'] = np.mean(ptu.get_numpy(qf_loss))
            self.train_statistics['VF Loss'] = np.mean(ptu.get_numpy(vf_loss))
            self.train_statistics['Policy Loss'] = np.mean(ptu.get_numpy(
                policy_loss
            ))
            self.train_statistics.update(create_stats_ordered_dict(
                'Q Predictions',
