...
from rlkit.core.popart import PopArtLayer
...
class PopArtFlattenMlp(PyTorchModule):
    def __init__(
            self,
            hidden_sizes,
            output_size,
        ...
        self.popart_layers = [PopArtLayer() for task in range(num_tasks)]


    def forward(self, input, task_idx=None, normalized=False):
        # 
        h = input
        for i, fc in enumerate(self.fcs):
            h = fc(h)
        ...
        normalized_output = self.last_fc(h)
        if normalized:
